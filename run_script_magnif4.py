...
for name in names:
    ...
    for s in s_:
        if not use_pretrained_model:
            time_E,time_M=funs.EM_PCA.run_MM('_joint_downsampled_'+name,s,learn_sigma_sq=False,palm_steps=300)
            times_E[count_out,count_in]=time_E
            times_M[count_out,count_in]=time_M
            time_E,time_M=funs.EM_PCA.run_MM('_joint_downsampled_'+name,s,learn_sigma_sq=True,palm_steps=300)
